...
def create_input(app_in, parent_frame, coil_idx, param_id):
    param = app_in.in_parameters[param_id]
    unit = app_in.units[param_id]

    # Current value from the dictionary keyed by (coil_idx, param_idx)
    value = app_in.values[(coil_idx, param_id)]
    ...
    slider = ctk.CTkSlider(frame, from_=0, to=app_in.top_values[param_id])
    slider.set(value)
    slider.pack(pady=10, fill="x", padx=10)

    def on_slider(val):
        val = int(float(val))
        app_in.values[(coil_idx, param_id)] = val
        entry.delete(0, "end")
        entry.insert(0, str(val))
        input_sync(app_in, coil_idx, param_id, val)
    ...
    def on_entry(_):
        try:
            ...
            if val > app_in.top_values[param_id]:  # update max if new value exceeds previous
                app_in.show_message(f"New max for {param}: {val}")
                app_in.top_values[param_id] = val
                for column_id in range(app_in.in_columns):
                    app_in.inputs[(column_id, param_id)]["slider"].configure(to=val)

            input_sync(app_in, coil_idx, param_id, val)

        except ValueError:
            entry.delete(0, "end")
            entry.insert(0, str(app_in.values[(coil_idx, param_id)]))
