...
class MSMLinear():
    ...
    def _compute_msm_force(self):
        ...
        for i in range(self.msm_elements_cnt):
            ...
            dyn_mass = tb_velocity**2 + (
                        utils.msm_mass / (utils.ro * utils.msm_A0) + self.data.qpos[self.tooth_joint_id_list[i]]
...
class MSM_Environment(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_set_cnt = utils.features_frames_stacked  # number of additional consecutive observation steps to be stuck together (will be added to the main observations)
